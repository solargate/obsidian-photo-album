...
function getAlbumCacheDir(plugin: Plugin, folder: string): string {
	return getPluginCacheDir(plugin) + '/' + folder;
}

function getCacheFileName(originalPath: string): string {
...
function getCachePath(plugin: Plugin, folder: string, originalPath: string): string {
	return getAlbumCacheDir(plugin, folder) + '/' + getCacheFileName(originalPath);
}

async function ensureCacheDir(app: App, cacheDir: string): Promise<boolean> {
...
function resizeImage(arrayBuffer: ArrayBuffer, size: number): Promise<Blob> {
...
async function generateThumbnail(app: App, originalPath: string, cachePath: string, size: number): Promise<boolean> {
	try {
		const data = await app.vault.adapter.readBinary(originalPath);
		const blob = await resizeImage(data, size);
...
export class ThumbnailCache {
	static async getOrGenerateThumbnails(plugin: Plugin, app: App, folder: string, files: Array<{ path: string; mtime: number }>, thumbnailSize: number): Promise<Map<string, string>> {
		const cacheDir = getAlbumCacheDir(plugin, folder);
		const success = await ensureCacheDir(app, cacheDir);
		...
		for (const file of files) {
			const cachePath = getCachePath(plugin, folder, file.path);
			...
			if (exists) {
				...
				needGenerate.push({ originalPath: file.path, cachePath, cacheFileName: getCacheFileName(file.path) });
			}
		}

		for (const item of needGenerate) {
			const success = await generateThumbnail(app, item.originalPath, item.cachePath, thumbnailSize);
			if (success) {
	...
	static async invalidateAllCache(app: App, plugin: Plugin): Promise<void> {
		const cacheDir = getPluginCacheDir(plugin);
		const exists = await app.vault.adapter.exists(cacheDir);
		if (exists) {
